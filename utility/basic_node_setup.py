...
class Setup():
    ...
    def create_vm(self):
        ...
        if flag == 1:
            ...
            sys.stderr.write(
                "VM setup time out for " + vm + "Check connection settings or increase wait time and try again\n")


def main():
    parser = argparse.ArgumentParser(
        description='VM spawner for pipeline processes.  Sets up vm for sample analysis and attaches sotrage references.  BE SURE TO DOUBLE CHECK VM AND CINDER IDS OF CONFIGURATION FILES BEFORE PROCEEDING!')
    parser.add_argument('-id', '--BID', action='store', dest='bid', help='Project Bionimbus ID')
    parser.add_argument('-j', '--json', action='store', dest='config_file',
                        help='JSON config file with snapshot ids and set up params')
    parser.add_argument('-w', '--wait', action='store', dest='wait',
                        help='Wait time before giving up on spawning an image.  Recommended value 600 (in seconds)')
