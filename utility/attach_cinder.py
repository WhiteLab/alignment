...
def attach_cinder(sid, vid, bid, size, vip, wait):
    cname = "REFS_" + bid
    sys.stderr.write(
        date_time() + "Creating cinder volume " + cname + " using snapshot ID " + sid + "to vm with ID " + vid + "\n")
    ...
    if flag == 1:
        ...
        sys.stderr.write(
            "Volume setup timeout for " + cname + "Check connection settings or increase wait time and try again\n")
    return flag
