...
sub print {
  ...
  print SCRIPT "$main::JAVA -Xmx8g -jar $main::PICARD/MarkDuplicates.jar CREATE_INDEX=true TMP_DIR=$main::PICARD_TMP REMOVE_DUPLICATES=true ASSUME_SORTED=true MAX_FILE_HANDLES_FOR_READ_ENDS_MAP=500 INPUT=$main::sample.clip.srt.bam OUTPUT=$main::sample.clip.rmdup.srt.bam METRICS_FILE=$main::sample.clip.rmdup.srt.metrics VALIDATION_STRINGENCY=LENIENT > $main::LOGS_DIR/$main::sample.picard.rmdup.pe.log 2>&1\n";
