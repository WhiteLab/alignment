...
def print_header_cc(tbl,ccvsn,ranges):
    tbl.write("BionimbusID\tDate\tMachine\tRun\tBarCode\tLane\tread_length\ttotal_reads\tpost_align_reads\tfraction_aligned\tpost_rmdup_reads\tfraction_rmduped\ttarget_size\taligned_bp_ot\tfraction_aligned_bp_ot\tfraction_sequenced_bp_ot\tmedian_insert_size\tmedian_absolute_deviation\tmean_insert_size\tinsert_standard_deviation\tdate_aligned\t" + ccvsn + "_t1_average_x_coverage\t" + ccvsn + "_t1_%covered_at_average\t")
    for r in ranges:
        r*=100
        r=str(int(r))
        tbl.write(ccvsn + "_t1_" + r + "%\t")
    tbl.write(ccvsn + "_t1_zero_cov_bp\t" + ccvsn + "_t1_%zero_cov\t" + ccvsn + "_t2_average_x_coverage\t" + ccvsn + "_t2_%covered_at_average\t")
    for r in ranges:
        r*=100
        r=str(int(r))
...
def parse_qc(config_file,sample,cflag):
    ...
    if cflag == 'n':
        ...
        (t1_ts,t1_aln_bp_ot,t1_num_bp,t1_cov,t1_zero_cov,t1_pct_zero_cov,t1_avg_cov,t1_avg_ratio) = parseCoverage(bed_hist1,ranges)
        target_size += t1_ts
        aligned_bp_ot += t1_aln_bp_ot
        num_bp += t1_num_bp
        log(loc,date_time() + 'Parsing bedtools coverage file ' + bed_hist2 + '\n')
        (t2_ts,t2_aln_bp_ot,t2_num_bp,t2_cov,t2_zero_cov,t2_pct_zero_cov,t2_avg_cov,t2_avg_ratio) = parseCoverage(bed_hist2,ranges)
        target_size += t2_ts
        ...
        to_print = (tot_rds,post_aligned_reads,frac_aligned,rmduped_reads,frac_rmduped_reads,target_size,aligned_bp_ot,fraction_aligned_bp_ot,fraction_sequenced_ot,median_insert_size,median_absolute_deviation,mean_insert_size,insert_standard_deviation,date_aligned,t1_avg_cov,t1_avg_ratio)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-f','--flag',action='store',dest='cflag',help='\'y\' if whole genome, \'n\' if custom capture to provide per-tier coverage stats')
