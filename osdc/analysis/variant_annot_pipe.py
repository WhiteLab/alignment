...
def variant_annot_pipe(config_file,sample_pairs,wait,kflag,ref_mnt,wg,sm):
    ...
    if sm == 'n':
        ...
        if check==0:
            ...
            sys.stderr.write(date_time() + 'Merged bam file download failed.  Make sure container, object correctly configured in config file\n')
            exit(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-sp','--sample-pairs',action='store',dest='sample_pairs',help='Tumor/normal sample pair list')
    ...
    parser.add_argument('-k','--karyo',action='store',dest='kflag',help='Flag to perform karyotypic reordering of BAM files.  Only need if original reference used wasn\'t sorted in the manner. \'y\' to do so')
    parser.add_argument('-r','--reference',action='store',dest='ref_mnt',help='Directory references are mounted, i.e. /mnt/cinder/REFS_XXX')
    parser.add_argument('-wg','--whole-genome',action='store',dest='wg',help='\'y\' or \'n\' flag if whole genome or not.  will determine whether to flag for on/off target')
    parser.add_argument('-sm','--skip-merge',action='store',dest='sm',help='\'y\' or \'n\' flag to skip merge files.  Useful for repeating variant calls when BAMs were already merged, sorted, etc the first time')
    ...
    (sample_pairs,config_file,wait,kflag,ref_mnt,wg,sm)=(inputs.sample_pairs,inputs.config_file,inputs.wait,inputs.kflag,inputs.ref_mnt,inputs.wg,inputs.sm)
