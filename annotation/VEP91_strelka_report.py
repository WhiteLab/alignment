        ...
        if impact in rank_dict:
            ...
            for ann in rank_dict[impact]:
                ...
                if call_type == 'snv':
                    ...
                    str(tum_ref_ct), str(tum_alt_ct), tum_alt_pct, str(tn_ratio), snp_id, gnomAD_AF, gene, hgvsg, tx_id,
                    variant_class, effect, impact, biotype, codon, aa, tflag)) + '\n'

                else:
                    cur_var = '\t'.join((chrom, pos, ref, alt, snp_id, gnomAD_AF, gene, hgvsg, tx_id, variant_class,
...
def gen_report(vcf, c, ref_flag):
    ...
    if call_type == 'snv':
        out.write('chr\tpos\tref\talt\tnormal_ref_count\tnormal_alt_count\t%_normal_alt\ttumor_ref_count\t'
              'tumor_alt_count\t%_tumor_alt\tT/N_%_alt_ratio\tsnp_ID\tgnomAD_AF\tgene\tHGVSg\ttranscript_id\t'
              'variant_class_effect\teffect\timpact\tbiotype\tcodon_change\tamino_acid_change\ton/off-target\n')
    else:
        out.write('chr\tpos\tref\talt\tsnp_ID\tgnomAD_AF\tgene\tHGVSg\ttranscript_id\tvariant_class_effect\teffect'
                  '\timpact\tbiotype\tcodon_change\tamino_acid_change\ton/off-target\n')
    if ref_flag != 'n':
