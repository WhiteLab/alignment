            ...
            while j < len(pile[i]):  # j is position of code for base call
                ...
                # making the final assumption that if none of the above conditions are met, then it's an indel
                    ...
                    try:
                        ...
                        sys.stderr.write('Parse error size was ' + str(
                            size) + ' current base code ' + cur + ' chromosome ' + chrom + ' position ' + str(
                            pos) + ' in sample ' + samp_list[k] + ' column index ' + str(i) + ' string index ' + str(
                            j) + '\n')
                        exit(3)
                    j = j + 2 + int(size)
                    base_call(cov, pile, cur, samp_list[k], chrom, pos, i, k, m)
                ...
                if call == 0:
                    sys.stderr.write(
                        'A base call was missed.  Check logic or input! current base code ' + cur + ' chromosome ' + chrom + ' position ' + str(
                            pos) + ' in sample ' + samp_list[k] + ' column index ' + str(i) + ' string index ' + str(
                            j) + '\n')
                    exit(3)
                m += 1
        ...
        for chrom in sorted(cov[samp]):
            for pos in sorted(cov[samp][chrom]):
                ...
                for base in sorted(cov[samp][chrom][pos]['bp']):
                    ...
                    # calc mean score
                    cov[samp][chrom][pos]['bp'][base]['x'] = "{0:.2f}".format(
                        mean(cov[samp][chrom][pos]['bp'][base]['score']))
                    # calc std dev
                    if cov[samp][chrom][pos]['bp'][base]['ct'] >= 2:
                        cov[samp][chrom][pos]['bp'][base]['stdev'] = "{0:.2f}".format(
                            pstdev(cov[samp][chrom][pos]['bp'][base]['score']))
                    else:
                        cov[samp][chrom][pos]['bp'][base]['stdev'] = 'NA'
                if tot_check != int(cov[samp][chrom][pos]['tot']):
                    sys.stderr.write(
                        'Per base coverage total (' + str(tot_check) + ') does not equal total coverage ( ' +
                        cov[samp][chrom][pos][
                            'tot'] + ') of position (' + pos + ') for sample ' + samp + '.  Try using math.\n')
                    exit(69)
                ...
                if pos in cov[samp][chrom]:
                    for base in cov[samp][chrom][pos]['bp']:
                        ...
                        if base in index[samp][chrom][pos]['mut']:
                            flag = 'Y'
                        sys.stdout.write("\t".join((samp, chrom, str(pos), gene, ref, base,
                                                    str(cov[samp][chrom][pos]['bp'][base]['ct']),
                                                    str(cov[samp][chrom][pos]['bp'][base]['x']),
                                                    str(cov[samp][chrom][pos]['bp'][base]['stdev']), flag)) + '\n')
                else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Variant pre-report using mpileup from samtools module.  Typically used with merged bams gives coverage report of mutation hot spots of interest.',
        add_help=True)
    ...
    parser.add_argument('-p', '--position', action='store', dest='pos',
                        help='Tab-separated position list with header Chromosome position ref_base mut_base gene_sym')
    parser.add_argument('-j', '--json', action='store', dest='config_file',
                        help='JSON config file with tool and reference locations')
    parser.add_argument('-r', '--ref_mount', action='store', dest='ref_mnt',
